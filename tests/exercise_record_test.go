...
func TestGetExercises(t *testing.T) {
	...
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("GET", "/api/exercises/1", nil)
	router.ServeHTTP(w, req)

	r.Equal(200, w.Code, w.Body.String())
	var resp models.ExerciseRecord
	r.NoError(json.Unmarshal(w.Body.Bytes(), &resp))
	r.Equal(now.Unix(), resp.CreatedAt.Unix())

	w = httptest.NewRecorder()
	req, _ = http.NewRequest("GET", "/api/exercises/404", nil)
	router.ServeHTTP(w, req)
	r.Equal(404, w.Code)
}
